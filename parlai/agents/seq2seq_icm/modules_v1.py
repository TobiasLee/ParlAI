...
class ICM(nn.Module):
    """Compute intrinsic reward given

    1. current state s_t
    2. previous state s_{t-1}
    3. action (vocab distribution)  a_{t-1}

    There are two sub modules:
    1. inverse dynamics model:
        mapping states to feature space f(s_t), f(s_{t-1}),
        then predict action \hat a_t given the mapped features
        reward_i = KL loss (a_t, \hat a_t )

    2. forward model:
        given f(s_{t-1}) and action a_{t-1}, predict \hat f(s_t)
        reward_f = l2_norm ( f_{s_t}  - \hat f(s_t))
    """

    def __init__(self, hidden_size, feature_size, vocab_size, beta=0.2, inv_hidden_size=256):
        """assuming that we are using LSTM here"""
        self.inv_mapping = nn.Linear(hidden_size, feature_size)
        # concat two features -> action space (vocab )
        self.inv_hidden = nn.Linear(feature_size, inv_hidden_size)
        self.action_predictor = nn.Linear(inv_hidden_size, vocab_size)
        self.fwd = nn.Linear(feature_size + vocab_size, feature_size)
        self.act = nn.Tanh()
    ...
    def forward(self, h_t, h_t_1, vocab_scores):
        # de-tuple from LSTM hidden state
        ht, ct = h_t
        ht1, ct1 = h_t_1
        # inverse dynamic model
        ft = self.act(self.inv_mapping(ht))  # f(h_t)
        ft1 = self.act(self.inv_mapping(ht1))  # f(h_{t-1})
        action_scores = self.action_predictor(
            self.act(
                self.inv_hidden(torch.cat((ft, ft1), dim=1))
            ))  # batch_size, vocab_size
        action_prob = self.log_softmax(action_scores, dim=1)
        vocab_prob = self.softmax(vocab_scores)
        # as required , the input is supposed to take log-prob, while the target is supposed only prob
        inv_reward = self.kl_loss(action_prob, vocab_prob)

        # forward
        predicted_ft = self.act(self.fwd(torch.cat((ft1, vocab_scores), dim=1)))  # feature_size
        forward_reward = self.l2_loss(ft, predicted_ft)

        return self.beta * inv_reward + (1 - self.beta) * forward_reward
...
class Seq2seq(nn.Module):
    ...
    def _decode_forced(self, ys, encoder_states):
        ...
        if self.attn_type == 'none':
            ...
            score, prob = self.output(output)
            rt = self.icm(hidden, h_prev, prob)
            rewards.append(rt)
    ...
    def _rank(self, cands, cand_inds, encoder_states):
        ...
        for batch_idx in range(len(cands)):
            ...
            score, rewards = self._decode_forced(curr_cs, cur_enc_states)
    ...
    def forward(
        ...
        if ys is not None:
            # use teacher forcing
            scores, rewards = self._decode_forced(ys, encoder_states)
        else:
            scores, rewards = self._decode(encoder_states, maxlen or self.longest_label)
        ...
        return scores, cand_scores, encoder_states, rewards
...
class OutputLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        return scores, probs # probs is used for ICM module
