...
class Seq2seqAgent(TorchAgent):
    ...
    def add_cmdline_args(cls, argparser):
        ...
        agent.add_argument(
            ...
            type=bool,
            default=False,
            help="use reinforcement learning reward to update agent"
        )

        agent.add_argument(
            '-hs_q',
            '--hidden_size_approximator',
            type=int,
            default=128,
            help="feature size of RNN approximator"
        )
        ...
        agent.add_argument(
            ...
            type=int,
            default=32,
            help="latent variable size, uniform distribution"
        )
    ...
    def __init__(self, opt, shared=None):
        ...
        if opt.get('numsoftmax', 1) > 1:
            ...
            self.dis_loss = nn.CrossEntropyLoss(size_average=False)
        else:
    ...
    def train_step(self, batch):
        ...
        try:
            ...
            # two mode: alternate between mle and rl or add the loss directly
            if self.opt['rl']:
                one_hot_z = out[3]
                qz_logits = out[-1]
                # update discriminator
                discriminator_loss = self.dis_loss(qz_logits, one_hot_z)

                # rl_reward is contract to cross entropy
                # question: no need monte carlo ?
                log_p_z = torch.mul(torch.ones(batchsize),
                                    torch.log(1.0 / self.opt['latent_size']))  # [batch_size]
                # batch_size x latent_size
                rl_reward = F.log_softmax(qz_logits) - F.log_softmax
            ...
            self.metrics['mle_loss'] += loss.item()
