...
class Seq2seq(nn.Module):
    """Sequence to sequence parent module with RL ."""
    ...
    def __init__(
        ...
        self.sampler = Categorical([1.0/ self.latent_size for _ in range(latent_size)])
        self.z_embedding = nn.Embedding(self.latent_size, hiddensize)

        # almost same setting with Encoder
        self.approximator = SimpleDiscriminator(hiddensize, latent_size)
    ...
    def _sample_z(self):
        ...
        z_one_hot = torch.zeros(self.latent_size)
        z_one_hot[z] = 1
        return z, z_one_hot

    def _get_z_embedding(self, z):
    ...
    def forward(
        ...
        # here we use random sampled z and encoder state to decode sentence
        z, one_hot_z = self._sample_z()
        embedded_z = self.z_embedding(z)  # batch_size , hidden_size

        # directly added to encoder_states
        encoder_states[1] += embedded_z
        ...
        if ys is not None:
            # use teacher forcing
            scores, decoder_hidden = self._decode_forced(ys, encoder_states)
        else:
            scores, decoder_hidden = self._decode(encoder_states, maxlen or self.longest_label)

        # given last hidden_state of decoder, infer the prob of z
        qz_logits, qz_prob = self.approximator(decoder_hidden)
        ...
        return scores, cand_scores, encoder_states, one_hot_z, qz_logits, qz_prob
...
class RNNApproximator(nn.Module):
    def __init__(
        ...
        if shared_rnn is None:
            ...
            self.output_layer = nn.Linear(hiddensize, latent_size)
        elif bidirectional:
            raise RuntimeError('Cannot share decoder with bidir encoder.')
        else:
            self.rnn = shared_rnn
        ...
        if shared_output is None:
            ...
            self.output_layer = shared_output

    def forward(self, xs):
        """get latent variable distribution given the generated tokens
        :xs
        xs: input tokens

        :return
        logits: original distribution logits over latent size
        prob
        ...
        logits = self.output_layer(hidden[0])  # only use h_n to infer the logits
        prob = F.softmax(logits, dim=1)
        return logits, prob


class SimpleDiscriminator(nn.Module):
    """ A simple FFN to infer the distribution given last hidden state of the decoder
    ...
    def __init__(self,
        ...
        self.act = F.tanh()
        self.ffn2 = nn.Linear(hidden_size, latent_size)

    def forward(self, decoder_states):
        h_t, c_t = decoder_states  #
        h1 = self.ffn1(h_t)  # batch_size, hidden_size * 2
        a1 = self.act(h1)
        logits = self.ffn2(a1)
...
class RNNDecoder(nn.Module):
    ...
    def forward(self, xs, hidden=None, attn_params=None):
        ...
        :returns:           output state(s), hidden state.
                            output state of the decoder. for an RNN, this is
