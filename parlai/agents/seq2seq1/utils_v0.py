...
def maintain_dialog_history(history, observation, reply='',
                            ...
                            splitSentences=False, separate_query=False):
    ...
    def parse(txt, splitSentences, from_observation=False):
        if dict is not None:
            txt_list = txt.split('\n')
            if splitSentences:
                vec = [dict.txt2vec(t) for t in txt_list]
                # from observation or from label/reply
                if from_observation:
                    qv = dict.txt2vec(txt_list[-1])
            else:
            ...
            # TODO without dict should be consistent with the above case
            return [txt]

    def parse_divide_query(txt, splitSentences,):
        if dict is not None:
            txt_list = txt.split('\n')
            if splitSentences:
                vec = [dict.txt2vec(t) for t in txt_list]
                ...
                if len(txt_list) == 1:
                    ...
                    add_info = [dict.txt2vec(t) for t in txt_list[:-1]]
            else:
                vec = dict.txt2vec(txt)
                qv = dict.txt2vec(txt_list[-1])
                if len(txt_list) == 1:
                    ...
                    add_txt = ' '.join(txt_list[:-1])
                    add_info = dict.txt2vec(add_txt)
            return vec, qv, add_info
        else:
            # TODO without dict should be consistent with the above case
            return [txt]
    ...
    if 'text' in obs:
        ...
        if not separate_query:
            text_vec, history['query'] = parse(obs['text'], splitSentences, from_observation=True)
            history['dialog'].extend(text_vec)
        else:
            # 23:51 4.23  add_info is the remain info after query is divided from the xs
            text_vec, history['query'], add_info = parse_divide_query(obs['text'], splitSentences,)
            history['query_not_included'] = history['dialog'].copy()
            history['dialog'].extend(text_vec)
            if add_info is not None:
                history['query_not_included'].extend(add_info)
    ...
    if not separate_query:
        return history['dialog'], history['query']
    else:
        return history['dialog'], history['query'], history['query_not_included']
...
class PaddingUtils(object):
    ...
    def pad_text(cls, observations, dictionary,
                 ...
                 truncate=None, is_not_cand=True, separate_query=False):
        ...
        ind_sorted = sorted(range(len(x_lens)), key=lambda k: -x_lens[k])  # [3,0,2,1] the 3's position of the origin qs is longest
        ...
        if is_not_cand:
            query = [deque(ex['query']) for ex in exs] # align with  sorted context
            q_lens = [len(q) for q in query]
            # q_index_sorted list of index e.g. [3,0,2,1] the 3's position of the origin qs is longest
            q_index_sorted = sorted(range(len(q_lens)), key=lambda k: -q_lens[k])
            q_invert_index = []  # e.g. [1,3,2,0]
            # q_invert_index e.g. [1,3,2,0]
            for i in range(len(q_lens)):
                for j in range(len(q_index_sorted)):
                    if q_index_sorted[j] == i:
                        q_invert_index.append(j)
            query = [query[k] for k in q_index_sorted]
            max_q_len = max(q_lens)
            # pad qs with zeros
            if dq:
                qs = [q if len(q) == max_q_len else q + deque((null_idx,)) * (max_q_len - len(q)) for q in query]
            else:
                qs = [q if len(q) == max_q_len else q + [null_idx] * (max_q_len - len(q)) for q in query]
            if separate_query:
                # similar to qs
                qni = [deque(ex['query_not_included']) for ex in exs]  # query not included in context
                qni_lens = [len(qni_item) for qni_item in qni]
                qni_index_sorted = sorted(range(len(qni_lens)), key=lambda k: -qni_lens[k])
                # to remind the sorted qni order to xs
                qni_invert_index = []
                for i in range(len(qni_lens)):
                    for j in range(len(qni_index_sorted)):
                        if qni_index_sorted[j] == i:
                            qni_invert_index.append(j)

                qni = [qni[k] for k in qni_index_sorted]
                max_qni_len = max(qni_lens)
                # pad qs with zeros
                if dq:
                    qni = [qi if len(qi) == max_qni_len else qi + deque((null_idx,)) * (max_qni_len - len(qi)) for qi in qni]
                else:
                    qni = [qi if len(qi) == max_qni_len else qi + [null_idx] * (max_qni_len - len(qi)) for qi in qni]
        ...
        if is_not_cand:
            if separate_query:
                return xs, qs, qni, ys, labels, valid_inds, q_invert_index, qni_invert_index, end_idxs, y_lens
            else:
                return xs, qs, ys, labels, valid_inds, q_invert_index, end_idxs, y_lens
        else:
